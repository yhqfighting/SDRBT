...
class BlockRecurrentAttention(nn.Module):
    ...
    def forward(
        ...
        self_attn_pos_emb = self.rotary_pos_emb(seq_len_list)

        state_len_list=[0]
        state_len_list.append(self.state_len)
        state_pos_emb = self.rotary_pos_emb(state_len_list)
